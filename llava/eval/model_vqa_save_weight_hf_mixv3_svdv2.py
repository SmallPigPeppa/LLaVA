...
def filter_delta(delta, retain_ratio=0.9, scale_factor=0.5):
    """
    对 delta 参数进行特征值分解，并过滤特征值，仅保留累积贡献达到 retain_ratio 的部分。
    选出的特征值乘以指定系数 scale_factor，其他部分保持不变。
    如果 total_variance 为 0，则直接返回原始 delta。
    """
    # Flatten delta to 2D matrix
    original_shape = delta.shape
    ...
    for i in selected_indices:
        adjusted_S[i] *= scale_factor

    # Reconstruct filtered delta using adjusted singular values
    filtered_delta = (U @ torch.diag(adjusted_S) @ Vh)
...
def load_and_mix_models(model_path_a, model_path_b, mix_ratio=0.5, retain_ratio=0.9, scale_factor=0.5):
    disable_torch_init()
    ...
    with tqdm(total=len(param_names), desc="Parameter Fusion Progress") as pbar:
        for name, param in param_names:
            if name in state_dict_b:
                ...
                filtered_delta = filter_delta(delta, retain_ratio=retain_ratio, scale_factor=scale_factor)
...
def save_mixed_model(args, mix_ratio=0.5, retain_ratio=0.9, scale_factor=0.5):
...
if __name__ == "__main__":
    ...
    save_mixed_model(args, args.mix_ratio, args.retain_ratio, args.scale_ratio)
