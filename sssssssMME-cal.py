...
class calculate_metrics:
    ...
    def parse_pred_ans(self, pred_ans):
        """
        Parse the predicted answer and map it to 'yes', 'no', or 'other'.
        Handles additional cases like 'true' and 'false'.
        """
        pred_ans = pred_ans.strip().lower()  # Ensure case-insensitivity and remove extra spaces
        pred_label = None

        if pred_ans in ["yes", "no"]:
            pred_label = pred_ans
        elif pred_ans in ["true", "false"]:  # Handle true/false cases
            pred_label = "yes" if pred_ans == "true" else "no"
        else:
            prefix_pred_ans = pred_ans[:4]  # Check for prefixes
